        ...
        >
          ...
          <div class="offcanvas-body my-auto mx-0 flex-grow-0">
            <div class="col-md-12">
              <div class="">
                <div class="form-floating">
                  ...
                  />
                  <label for="floatingInput">DPCR Details</label>
                  <span *ngIf="isBlankDetails" class="text-danger">DPCR Details is required.</span>
                </div>
              </div>
              <br />
            </div>
            <div class="col-md-12 mt-2" *ngIf="isAddDpcr">
              <label for="largeSelectYear" class="form-label"
                >Select year</label
              >
              <select
                id="largeSelectYear"
              ...
              >
                <ng-container *ngFor="let year of listYear">
                  <option value="{{ year }}" [selected]="year === dpcrObj.year">
                    {{ year }}
                  </option>
                </ng-container>
              </select>
            ...
            <div class="col-md-12 mt-2">
              <label for="largeSelectPeriod" class="form-label"
                >Select period</label
              >
              <select
                id="largeSelectPeriod"
                class="form-select form-select-lg"
              ...
              >
                <ng-container *ngFor="let period of periods">
                  <option
                    value="{{ period.period }}"
                    [selected]="dpcrObj.semester === period.period"
                  >
                    {{ period.desc }}
                  </option>
                </ng-container>
...
export class CanvasTargetDpcrComponent implements OnInit {
  ...
  periods = [
    { period: 1, desc: '1st Sem' },
    { period: 2, desc: '2nd Sem' },
  ];
  listYear: any = [];
  ...
  isBlankDetails: boolean = false;
  Submit() {
    if (this.dpcrObj.details === '' || this.dpcrObj.details === null || this.dpcrObj.details === undefined) {
      this.isBlankDetails = true;
    } else {
      this.isBlankDetails = false;

      if (this.dpcrObj.year === undefined || this.dpcrObj.year === null) {
        this.dpcrObj.year = new Date().getFullYear() - 1;
      }
      if (
        this.dpcrObj.semester === undefined ||
        this.dpcrObj.semester === null
      ) {
